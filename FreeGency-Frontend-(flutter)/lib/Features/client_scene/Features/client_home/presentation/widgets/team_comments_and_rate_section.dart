...
class TeamCommentsAndRateSection extends StatefulWidget {
  final TeamsModel team;

  const TeamCommentsAndRateSection({
    super.key,
    required this.team,
  });

  @override
  State<TeamCommentsAndRateSection> createState() =>
      _TeamCommentsAndRateSectionState();
}

class _TeamCommentsAndRateSectionState
    extends State<TeamCommentsAndRateSection> {
  ...
  void initState() {
    ...
    _reviewsCubit.getReviews(_teamId);
  }
                ...
                if (state is GetReviewsLoading) {
                  ...
                  return Column(
                    children: state.reviews.map((review) {
                      // Convert API review to the format expected by UsersCommentsItem
                      final comment = {
